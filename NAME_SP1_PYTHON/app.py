...
Probleme der Aufgabe:

Wie verstecke ich Passwörter im Code?

5 Endungen Erstellen GET, POST, DELETE

Wie mache ich Requests sichtbar?

Wenn man mit Postman einen Request an die URL sendet, erkennt er den Body nicht als Strings/array, sondern als "None".



Lösung: 

Mit sogenannten Environments-Variablen die in der Configuration festgehalten werden können.
Oder über eine .gitignore-Struktur

3 Application-Pfade mit den richtigen URL's.

Mit Postman. Spezieller Focus auf den Header, Body

Wenn man im Postman eine request an die URL sendet, benötigt diese einen Header -> Content-Type application/json
Um den Body als Arrays/String lesen zu können. Dieser muss manuel in den Header der Request hinzugefügt werden.

To-Dos für die nächsten Wochen:
Bessere alternative zu Flask: FastAPI
.gitignore verstehen
Ordner-Strukturen anschauen
