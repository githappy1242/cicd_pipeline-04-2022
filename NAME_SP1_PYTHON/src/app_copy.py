...
def db_get(sql, values): #Definiert Methode db_get mit Parameter 1 und 2, values??
...
def books():
  ...
  if request.method == 'POST': #Post funktioniert noch nicht, Nonetype object is not subscriptable
    ...
    result = db_get(sql, (body['title'], body['year_written'], body['author']))
  return json.dumps(result)



@app.route('/api/v1/book/<int:id>', methods=['GET', 'POST', 'DELETE']) #DELETE in Methods
def single_book(id):
    result = None
    if request.method == 'GET':
    ...
    if request.method == 'POST':
        body=request.json #Postman sendet einen POST request an meinen Code. In diesem
        #ist eine Body, den er nicht lesen kann. Finde heraus warum.
        print(body)
        sql = 'INSERT INTO books (title, year_written, author) VALUES (%s, %s, %s, %s)' #SQL Statement
        result = db_get(sql, body['title'], body['year_written'], body['author'])
        return json.dumps(result)
...
def years(year_written): #years wird definiert, year_written wird übergeben
    ...
    if request.method == 'GET': #Wenn GET, dann
        sql = 'SELECT * FROM books WHERE year_written=%s' #Einträge werden aus der Datenbank abgerufen
        result = db_get(sql, year_written) #ergebnis wird ausgeworfen mit den Variablen
    return json.dumps(result)
