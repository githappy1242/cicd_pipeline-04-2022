...
@app.route('/api/v1/book/<int:id>', methods=['GET', 'DELETE'])
def single_book(id):
    result = None
    if request.method == 'GET':
...
def years(year_written): #years wird definiert, year_written wird übergeben
    ...
    if request.method == 'GET': #Wenn GET, dann
        sql = 'SELECT * FROM books WHERE year_written=' + year_written #Einträge werden aus der Datenbank abgerufen
